...
Example:logger.begin(loggerCallback([](uint8_t level, const char *levelN, const char *str, const char *strN) {
  // Custom handling
}));
...
Example:logger.begin(Serial, loggerCallback([](uint8_t level, const char *levelN, const char *str, const char *strN) {
  Serial.println(strN);
}));
...
Example:logger.setLogLevel(Loglevel::debug); // Allow err, info, and debug messages
...
Example:Loglevel level = logger.getLogLevel();
...
Example:logger.setEnabled(0); // Disable logging
...
Example:logger.error("Error code: %i, %s", 404, "Not found");
...
Example:logger.error(F("Error: %s"), "Critical");
...
Example:logger.info("Temperature: %f°C", 25.5);
...
Example:logger.info(F("System running"));
...
Example:logger.debug("Sensor value: %i", 1023);
...
Example:logger.debug(F("Debug mode: %s"), "ON");
...
Customization: Define before including Logger.h:#define LOG_BUFFER_SIZE 512
#include <Logger.h>
